        ...
        <thead className="bg-[#0b1437]">
          <tr>
            {headers.map((h) => (
              <th
                key={h}
                className="text-left text-xs font-semibold text-gray-300 tracking-wide px-4 py-3 border-b border-[#1f2d5e]"
              >
                {h}
              </th>
            ))}
        ...
        <tbody className="bg-[#111c44]">
          {rows.length === 0 ? (
            ...
            rows.map((r, i) => (
              <tr
                key={i}
                className="border-b border-[#1f2d5e] hover:bg-[#1a2755] transition"
              >
                {r.map((cell, j) => (
