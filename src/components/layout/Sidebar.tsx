...
export function Sidebar({ open, onClose }:{ open:boolean; onClose:()=>void }) {
  ...
  const isAdmin = true; // replace with role logic if needed

  const sections = [
    { title: "Core Operations", links: [
    ...
    { title: "Business Management", links: [
  ...
  return (
    <>
      <div className={`fixed inset-0 bg-black/50 z-30 lg:hidden transition-opacity ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`} onClick={onClose}/>
      <aside className={`fixed z-40 inset-y-0 left-0 w-72 bg-[#111c44] border-r border-[#1f2d5e] flex flex-col transform transition-transform ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
        <div className="p-5 border-b border-[#1f2d5e]">
          <h1 className="text-2xl font-extrabold tracking-tight"><span className="text-[#dc2626]">SMASH</span> <span>SCRAP</span></h1>
          ...
          {sections.map(sec=>(
            <div key={sec.title} className="mb-6">
              ...
              {sec.links.filter(l=>l.show).map(link=>(
                <NavItem key={link.to} to={link.to} label={link.label} active={loc.pathname===link.to}/>
              ))}
            </div>
          ))}
        </nav>
