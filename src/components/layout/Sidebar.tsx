...
export function Sidebar() {
  ...
  const sections = [
    { title: 'Core Operations', links: [
      ...
      { to: '/boxes', label: 'Inventory Management', show: canManage },
      { to: '/parts', label: 'Converter Builder', show: canManage },
  ...
  return (
    <aside className="w-64 bg-[#111c44] border-r border-[#1f2d5e] flex flex-col">
        ...
        {sections.map(sec => (
          <div key={sec.title} className="mb-6">
            ...
            {sec.links.filter(l => l.show).map(link => (
              <Link key={link.to} to={link.to} className={`block px-4 py-2.5 mb-1 rounded-lg text-sm font-medium transition ${loc.pathname === link.to ? 'bg-[#dc2626] text-white shadow-lg' : 'text-gray-300 hover:bg-[#1f2d5e]'}`}>
                {link.label}
              </Link>
            ))}
          </div>
